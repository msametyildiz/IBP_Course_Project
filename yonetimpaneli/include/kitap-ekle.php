<div class="content-wrapper">
    ...
    <section class="content">
        <div class="container-fluid">
          <div class="row">
            <div class="col-md-12">
              <a href="<?= SITE ?>kitap-liste/kitapkonulari" class="btn btn-info" style="float:right; margin-bottom=10px; margin-left:10px;"><i class="fa fa-bars"></i> LİSTE</a>
            </div>
          </div>
    ...
    if ($_POST) {
        if (!empty($_POST["konu"])) {
            ...
            $kontrol = $VT->VeriGetir("kitapkonulari", "WHERE konu=?", array($tablo), "ORDER BY ID ASC", 1); // TODO
            if ($kontrol != false) {
                ...
                $ekle = $VT->SorguCalistir("INSERT INTO kitapkonulari", "SET konu=?, selflink=?, durum=?, tarih=?", array($konu, $tablo, $durum,date("Y-m-d") ));
                $veri = $VT->VeriGetir("kitapkonulari", "WHERE selflink=?", array($tablo), "ORDER BY ID ASC");
                $sirano=$veri[0]["ID"];
                $guncelle = $VT->SorguCalistir("UPDATE kitapkonulari", "SET konu=?, selflink=?, durum=?, sirano=?, tarih=? WHERE selflink=?", array($konu, $tablo,$durum,$sirano ,date("y-m-d"), $tablo));
                if ($ekle != false) {
